...
class ApplicationControllerSpec extends PlaySpecification with Mockito {
  ...
  "The `authenticated` action" should {
    "redirect to login page if user is unauthorized" in new Context {
      new WithApplication(application) {
        ...
        redirectURL must contain(
          routes.ApplicationController.notAuthenticated.toString
        )

        val unauthorizedResult =
          route(app, addCSRFToken(FakeRequest(GET, redirectURL)))
            .getOrElse(throw FailureException(failure("required Some")))

        status(unauthorizedResult) must be equalTo OK
        contentType(unauthorizedResult) must beSome("text/html")
        contentAsString(unauthorizedResult) must contain(
  ...
  trait Context extends Scope {
    ...
    val identity = User(
      ...
      loginInfo = LoginInfo("google", "user@gmail.com"),
      firstName = None,
      lastName = None,
      fullName = None,
      email = None,
    ...
    implicit val env: Environment[DefaultEnv] =
      new FakeEnvironment[DefaultEnv](Seq(identity.loginInfo -> identity))
